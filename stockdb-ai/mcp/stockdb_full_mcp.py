# =============================================================================
# 上游文件拷贝（0.9.0 M4 SDK 桥）：仅复用其 stockdb_* 函数做参数归一化与 SDK 调用。
# 来源：hello245m/free-stockdb（MIT License）发布包的 调用方式/ai_mcp/stockdb_full_mcp.py
# 拷贝自本机原生目录（2026-08-16，上游 0.3.1 more-power 发行版）。
# 维护约定：上游升级后重新拷贝本文件 + 冒烟回归（见 docs/design/sdk-mcp-bridge.md §8）；
# 本文件不启动服务器（mcp 对象仅被本模块引用，不被 sdk_bridge 使用），
# 服务器启动/调度逻辑一律在 stockdb_mcp_server.py 与 sdk_bridge.py。
# =============================================================================
import asyncio
import datetime
import json
